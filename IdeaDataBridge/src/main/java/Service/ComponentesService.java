    ...
    public static void inserirComponentesNoBD(ConexoesController conexoes, ConexoesController conexoesMysql, Totem totem ) {
        ...
        if (hardwares.isEmpty()) {
            ...
            if (totem.getProcessador() != null) {
                ...
                conexoesMysql.getConMySql().update("insert into hardware(tipo,fkTotem) values(?,?)", TipoHardware.PROCESSADOR.getNome(),codigoTotem);
            }
            if (totem.getMemoria() != null) {
                ...
                conexoesMysql.getConMySql().update("insert into hardware(tipo,fkTotem) values(?,?)", TipoHardware.MEMORIA.getNome(),codigoTotem);
            }
            if (totem.getGrupoDisco() != null) {
                ...
                conexoesMysql.getConMySql().update("insert into hardware(tipo,fkTotem) values(?,?)", TipoHardware.DISCO.getNome(),codigoTotem);
            }
            try {
